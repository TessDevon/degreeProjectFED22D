...
export default function LoginModal() {
  ...
  const checkPassword = new RegExp(
    /^(?=.*[A-ZÅÄÖa-zåäö])(?=.*\d)[A-ZÅÄÖa-zåäö\d]{8,}$/
  );
  // Requires login with at least one capital letter and number, at least 8 characters and no spaces.
      ...
      if (checkNames.test(formData.lastname)) {
        if (checkEmail.test(formData.email)) {
          if (checkPassword.test(formData.password)) {
            if (
              formData.userImage != undefined &&
              checkImg.test(formData.userImage?.name)
            ) {
              saveNewUserData(
                formData.firstname,
                formData.lastname,
    ...
    <div className="signView">
      <Modal isOpen={showLoginModal} style={customStyles}>
        ...
        <div className="loginView">
          ...
          <StyledButtonCenter onClick={goToRegister}>
            {t("loginRegisterBtnText")}
          </StyledButtonCenter>
        </div>
      </Modal>
      <Modal isOpen={showRegisterModal} style={customStyles}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <StyledButtonRightsided>
            {t("registerRegistrationbtnText")}
          </StyledButtonRightsided>
        </form>
        <StyledButtonLeft onClick={goToLogin}>
